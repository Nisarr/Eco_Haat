                ...
                <div className="flex flex-wrap gap-4 items-center justify-between">
                    ...
                    <div className="relative">
                        <select
                            ...
                            className="appearance-none bg-white pl-4 pr-10 py-3 rounded-xl border border-gray-200 focus:border-green-500 outline-none cursor-pointer"
                        >
                            {materials.map(m => (
                    ...
                    <div className="flex gap-2">
                        <button
                            ...
                            className={`p-3 rounded-xl transition-colors ${viewMode === 'grid' ? 'bg-green-100 text-green-600' : 'bg-gray-100 text-gray-500'}`}
                        >
                            <Grid size={20} />
                        </button>
                        <button
                            onClick={() => setViewMode('list')}
                            className={`p-3 rounded-xl transition-colors ${viewMode === 'list' ? 'bg-green-100 text-green-600' : 'bg-gray-100 text-gray-500'}`}
                        >
                            <List size={20} />
                        </button>
                    </div>
                    ...
                    {products.map((product, index) => (
                        ...
                        >
                            <Link href={`/products/${product.id}`}>
                                <div className={`bg-white rounded-2xl border border-gray-100 overflow-hidden hover:shadow-lg transition-all duration-300 group ${viewMode === 'list' ? 'flex' : ''
                                    }`}>
                                    <div className={`${viewMode === 'list' ? 'w-48 h-48' : 'aspect-square'} bg-gray-100 relative overflow-hidden`}>
                                        ...
                                        {product.eco_rating && (
                                            <div className="absolute top-3 right-3 bg-white/90 backdrop-blur-sm px-2 py-1 rounded-full text-sm font-medium text-green-700">
                                                🌿 {product.eco_rating}%
                                            </div>
                                        )}
                                    </div>
                                    <div className={`p-4 ${viewMode === 'list' ? 'flex-1 flex flex-col justify-center' : ''}`}>
                                        <div className="text-xs text-green-600 font-medium mb-1">{product.material}</div>
